...
const CreateInternshipPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await internshipAPI.create(formData);
            ...
            <div className="flex items-center gap-3 p-4 bg-primary-50 border border-primary-200 rounded-lg">
              ...
              <label htmlFor="publish" className="flex-1">
                ...
                <p className="text-sm text-neutral-600">
                  {formData.publish
                    ? 'This internship will be visible to students right away'
                    : 'This internship will be saved as a draft. You can publish it later from "My Internships"'}
                </p>
              </label>
            </div>
          </section>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {formData.publish ? 'Create & Publish' : 'Save as Draft'}
                </>
              )}
            </button>
          </div>
